...
| 8 | Integrations & Open Platform | Connect calendars, CRMs, chat, webhooks | High | 2.5–4 wk | Power |
...
- **Modules affected:** leads (major), contacts, users (ownership), events, object-storage (attachments), audit.
- **Mobile impact:** Mobile pipeline/Kanban, my-leads, log-activity, add-note, view-timeline; attachment upload from camera/files.
- **Admin dashboard impact:** Upgraded `/admin/leads` (pipeline + Kanban + detail with timeline/notes/attachments/tags); ownership reassignment UI.
- **Backend impact:** Extend leads service/repo; new activities, notes, attachments, tags services; ownership-transfer logic; attachment storage via existing object-storage.
- **Database impact:** New tables — `lead_activities`, `lead_notes`, `lead_attachments`, `tags` + `lead_tags`, pipeline `stages` (per-tenant configurable); add `ownerId`/`teamId`/`stageId`/opportunity fields to `leads` (additive, nullable).
- **API impact:** New endpoints for stages, activities, notes, attachments, tags, ownership transfer; extend lead read/write. Static sub-paths registered before `/:id`.
...
## Phase 8 — Integrations & Open Platform

- **Objective:** Connect Card Scanner Pro to the tools customers already use and open it via API/webhooks.
- **Business value:** Stickiness + reach; leads/contacts/events flow into existing workflows; ecosystem play.
- **Features:** Microsoft 365 / Google Workspace, Outlook + Google Calendar, HubSpot / Salesforce / Zoho CRM, Slack / Microsoft Teams, Zapier, public REST API, webhooks.
- **Modules affected:** new integrations module, contacts/leads/events (sync), auth (OAuth to third parties), background-jobs (sync), notifications.
- **Mobile impact:** Surface synced data (calendar events, CRM status); trigger sync; minimal config (setup stays web).
- **Admin dashboard impact:** `/admin/integrations` — connect/disconnect, field mapping, sync status/logs, webhook + API-key management.
- **Backend impact:** OAuth connection management (use Replit integrations/connectors where available — check first, don't hardcode keys); sync engine via background jobs; webhook dispatch (signed, retried); public API surface + rate limiting.
- **Database impact:** New `integration_connections`, `sync_state`, `webhooks`, `webhook_deliveries`, `api_keys` tables.
- **API impact:** Integration CRUD, OAuth callbacks, sync triggers, webhook config, public versioned REST API + key auth.
- **Dependencies:** Phase 3 (CRM data to sync) strongly recommended; Phase 9 overlaps on API keys/security. Several connectors can be built in parallel once the integration framework exists.
- **Risks:** Third-party OAuth/credential security (use integrations system, never store raw keys unsafely); sync conflicts/duplication (idempotency + dedup); webhook retry storms; rate-limit abuse on public API; partner API changes.
- **Testing:** Per-connector sync with mocked APIs; OAuth flow; webhook signing + retry; public API auth + rate limit + tenant scoping; idempotent sync.
- **Complexity:** High. **Time:** 2.5–4 weeks (framework first, then connectors incrementally). **Mode:** Power.
...
- **Phase 8 (Integrations)** framework can start after Phase 3; individual connectors parallelize among themselves.
- **Phase 9 (Enterprise Admin)** shares API-key/security work with Phase 8 and is best **last**, since it administers everything the prior phases create.
...
This is the master plan for the rest of Stage 3. **No coding starts until you approve.** Tell me to proceed (and with which phase — I recommend **Phase 2**), or what to adjust.
