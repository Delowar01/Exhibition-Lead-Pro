...
| `test/b20c3-checkout-fence.test.ts` (3) | Correction 3: cold start + concurrent different-price switch (request parked between provider customer creation and its binding cannot open a session for its retired intent; safe conflict; peak open remote sessions = 1; one current intent = the replacement; retired row untouched; accurate customer-link / checkout audits); cold-start same-price concurrency (one provider customer under the stable key, one remote session, one intent, reuse); opposite fence ordering (binding committed, session creation delayed → the switch recovers and expires the id-less session through the recovery protocol; the delayed request cannot open another session or resurrect the intent); no provider call inside a transaction |
...
2. **Provider customer** (outside any transaction) with the stable key `customer:<sha256("customer|"+companyId)>`; then a short transaction links `stripe_customer_id` (409 `PROVIDER_CUSTOMER_MISMATCH` if another id is already linked). A retry after a local failure re-sends the same key and receives the same customer — never a second one. **Correction 3 — customer-binding fence:** inside that transaction the customer is bound to the intent *conditionally* (`bindIntentCustomer`: the row must still be `creating|open`, belong to this company **and** this canonical subscription, and carry no other customer) **before** the subscription link + audit `subscription.provider_customer_linked{checkoutId}` are written; if the fence does not match (a concurrent price switch retired the intent while the provider call was in flight) nothing is written and the request stops with 409 `CHECKOUT_IN_PROGRESS` — the retired row is never touched, its key never authorizes a session, no URL is returned. Only a bound intent may proceed to step 3.
...
1. *Recover the session id* (only when it is missing): if the intent has no provider customer, no session creation has been **authorized** yet (the fence binds the customer before any session call) — but a request may be in flight. Correction 3 therefore retires such an intent only through `retireUnboundIntent` (row still `creating|open` **and** customer **and** session both null, one short transaction); when that guarded update matches nothing, a binding won the race: the intent is re-read and the bound row continues through the recovery protocol below (replay with its key → persist → settle → expire) instead of being retired blindly. The fence and the guarded retirement touch the same row under PostgreSQL row locking, so exactly one of them commits first: binding first ⇒ the switch must close the (possibly in-flight) session at the provider; retirement first ⇒ the binding fails and the delayed request stops with a conflict. A request whose session attachment or link finds its intent closed concurrently also stops with 409 `CHECKOUT_IN_PROGRESS` (never `continue` on a stale view, never a dead URL). Otherwise replay the ORIGINAL creation (`checkout.sessions.create`) with the intent-derived key `checkout:<sha256("checkout|"+companyId+"|"+intentId)>` and parameters rebuilt from the durable intent + server configuration (customer, price mapping, return URLs, `client_reference_id`, opaque metadata, automatic-tax flag — nothing is stored; no URL, secret or PII). Same key + identical parameters → the same session (or it is created now if the original request never reached the provider); a parameter mismatch (configuration changed since), transient failure or timeout → 502 `PROVIDER_ERROR`, **intent stays current, no replacement**. The reply is used only for the session id.
