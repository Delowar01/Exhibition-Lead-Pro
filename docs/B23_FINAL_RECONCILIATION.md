...
### 1.6 G-6 backup inventory and isolated restore rehearsal (2026-09-18 23:12–23:17 UTC; runs 35404789095, 35405027051)

Read-only inventory and one restore into a disposable PostgreSQL 16 container on the VPS
(`ops/b23-g6-backup-rehearsal` = `f53c901` + ops files; nothing merged or deployed; the
live container, volume, env, schema and tenant data were re-verified unchanged afterwards).

| Item | Evidence | Status |
|---|---|---|
| Effective backup script | hosted `docker/scripts/backup-postgres.sh` identical to the tracked blob; `KEEP=7`, size floor only — no `gzip -t`, no temp-then-rename, no lock, no checksum file, no off-host step, no alerting | verified (as documented) |
| Schedule | cron daemon active but the deploy user's crontab, `/etc/crontab`, `/etc/cron.d`, `cron.{hourly,daily,weekly,monthly}` and both repo workflows contain no backup job; 20 system timers, none referencing the script (one unrelated `elite-one-desk-backup.timer` whose unit is not readable by the deploy user); `backup.log` absent; root's crontab not readable | **failed — nothing schedules the backup** (root crontab unknown) |
| Local backups | 7 files 2026-09-02 → 2026-09-15 (20,430 → 324,893 B), all gzip-valid, complete (`-- PostgreSQL database dump complete`), pg_dump 16.15, 64→68 `CREATE TABLE`, mode 600 in a 700 directory on `/` (not inside a Docker volume), no partial files | verified |
| Latest successful backup | `leadcapture-20260915-154510.sql.gz`, sha256 `5065668e…4462`, 79 h old at the rehearsal, schema fingerprint F0 (68 tables) — older than the live schema F2 (72 tables) | verified (stale relative to schema) |
| Off-host copy | no rclone/gsutil/aws/restic config for the deploy user; no off-host command in any readable schedule; app dev bucket lists 4 objects, 0 backup-like; Hostinger panel snapshots not verifiable from inside the VPS | **unknown / none evidenced** |
| Capacity gate | disk 68.4 GB free, 4.9 GB RAM available, `postgres:16-alpine` image present; no leftover rehearsal resources | passed |
| Isolated restore | container `g6-restore-<tag>` from the live container's image id, `--network none`, no published port, private volume, 640 MiB / 1 CPU / 256 pids, backup bind-mounted read-only; 1 referenced role re-created; `psql -v ON_ERROR_STOP=1 --single-transaction` exit 0, 0 stderr lines | **passed** |
| Verification | tables 68/68, foreign keys 141/141, indexes = dump `CREATE INDEX` + pk/unique constraints, extensions present; per-table counts identical to the dump's COPY blocks (68 tables, 3,206 rows; `companies=1 users=2 job_queue=3118 workflow_runs=0 export_runs=4 export_schedules=0 audit_logs=27 subscriptions=1`); read-only session refuses writes; orphan checks 0; newest `job_queue.enqueued_at` 2026-09-15 15:45:04 ≈ backup mtime | passed |
| Measurements | restore 867 ms, verification 587 ms, total 1,454 ms for a 1.2 MB dump; observed recovery point 79 h (measurements of this rehearsal, not a guarantee) | recorded |
| Cleanup | trap removed container, volume and work dir; re-check: 0 g6 containers/volumes/dirs; backup files (set, sizes, mtimes, sha256) unchanged; containers/volumes/images/env unchanged; `c4-verify` after: `f53c901`, F2, baseline md5 equal, readyz ok, 0 API errors | verified |
| Rollback readiness | `previous-deploy.sha` = `ff7b851` (present in the checkout, on `origin/develop`); no SHA-tagged images → rollback = rebuild (≈3.5 min per recent deploy); no schema change between `ff7b851` and `f53c901`; auto-rollback path never exercised | assessed only |

### 1.7 G-6 Correction 1 — fresh current-schema backup and backup automation (2026-09-18 23:38–23:41 UTC; runs 35406504223, 35406595433)

| Item | Evidence | Status |
|---|---|---|
| Preflight (read-only) | `f53c901` = `current-deploy.sha`, F2, readyz ok, same containers/volume, env sha256 prefix `8bc9ef9a2f8554bf`, db 16,391,191 B, 7 backups with sha256 unchanged since §1.6, no backup created since the rehearsal, no backup running, disk 68.4 GB / RAM 4.9 GB free | passed |
| Schedule uncertainty | no passwordless sudo for the deploy user → root's crontab **still unknown**; the other site's `elite-one-desk-backup.timer` (05:15 Asia/Riyadh = 02:15 UTC, `Persistent=yes`, last success 2026-09-18 02:15 UTC) is root-only on disk, but the manager reports its executable as `elite-one-desk-backup` with no reference to this stack, postgres or docker | resolved as far as readable |
| Fresh backup | the DEPLOYED script (`85c1266d…`) run once with `KEEP=8` at 23:40:35 UTC → `leadcapture-20260918-234035.sql.gz`, 358,248 B, mode 600, sha256 `9d51ae6d3bd3446c646abffff8fb872c1b7160310ec8d97077df56684f240693`, pg_dump 16.15, 72 `CREATE TABLE` (= live), 72 COPY blocks, 3,525 rows, completion marker; the 7 earlier files byte-identical (name, size, sha256, mtime); 8 retained | verified |
| Isolated restore of that file | same-image network-less container, `ON_ERROR_STOP` single transaction, exit 0, 0 errors; tables 72/72, FKs 148/148, indexes 241 = 152 + 89, rows identical to the dump (3,525; `companies=1 users=2 job_queue=3408 workflow_runs=0 billing_checkout_sessions=0 export_runs=4 audit_logs=28`); **restored fingerprint `ce55dfa2…` = live F2 seen at preflight**; newest `job_queue.enqueued_at` 2026-09-18 23:38:52; restore 914 ms + verify 594 ms; container/volume/work dir removed and absent; live stack, baseline md5 and fingerprint unchanged afterwards | **verified — the backup captures F2** |
| Durable fix (review branch `claude/b23-g6c1-backup-automation`) | hardened `docker/scripts/backup-postgres.sh` (flock, unique temp file + atomic publish, gzip/size/header/marker checks before publishing, sha256 sidecar, prune only after verification incl. sidecars, no overwrite, 600/700), new `backup-check.sh`, test harness with 25 assertions incl. a real restore into a disposable database, `docs/BACKUP_AND_RECOVERY.md` (proposed 03:15 UTC crontab for the deploy user, activation/rollback, alert, off-host design) | implemented, not deployed |
| Still open | schedule not installed; off-host copy not configured/verified; rollback not rehearsed | open |

### 1.8 G-6 Correction 2 — missed-run detection, checksum publication and the bridging backup (2026-09-22 09:59–10:05 UTC; runs 35713483467, 35713935672)

| Item | Evidence | Status |
|---|---|---|
| Preflight (read-only, 09:59:53 and 10:04:35 UTC) | `f53c901` = `current-deploy.sha`, `previous` = `ff7b851`, F2 `ce55dfa2…`, readyz ok, same three container ids and volume, env sha256 prefix `8bc9ef9a2f8554bf`, db 16,579,607 B, 8 backups with the sha256 values of §1.7, none created since the C1 capture, no backup running, disk 77.2 GB / RAM 4.9 GB free; root's crontab still unreadable (no passwordless sudo) → unknown; the other site's timer last ran 2026-09-22 02:15:01 UTC and still references nothing of this stack | passed |
| Bridging decision | newest complete backup `leadcapture-20260918-234035.sql.gz` was 82 h old (> 26 h) although its 72 tables matched the live count → `bridge_needed=yes existing_backups=8 keep_for_bridge=9` | bridge taken |
| Bridging backup | the DEPLOYED script (`85c1266d…`) run once with `KEEP=9` at 10:04:47 UTC → `leadcapture-20260922-100447.sql.gz`, 398,288 B (1,441,901 B uncompressed), mode 600, sha256 `6ebfda25638e99852a6a3be37a7ec0a4d46bb1e3381e5f9fe533780e54709959`, pg_dump 16.15, 72 `CREATE TABLE` (= live), 72 COPY blocks, 3,934 rows, completion marker; the 8 earlier files byte-identical (name, size, sha256, mtime); 9 retained | verified |
| Isolated restore of that file | same-image network-less container, `ON_ERROR_STOP` single transaction, exit 0, 0 errors; tables 72/72, FKs 148/148, indexes 241 = 152 + 89; rows identical to the dump (3,934; `companies=1 users=2 job_queue=3817 workflow_runs=0 billing_checkout_sessions=0 export_runs=4 audit_logs=28`); **restored fingerprint `ce55dfa2…` = live F2 at preflight and after**; newest `job_queue.enqueued_at` 2026-09-22 10:03:52; restore 1,341 ms + verify 1,042 ms; container/volume/work dir removed and absent; stack, baseline md5, env and live fingerprint unchanged afterwards | **verified — captures F2** |
| Automation fixes (review branch `claude/b23-g6c2-backup-guard`) | (1) post-run freshness limit 4 h for the 03:45 and 04:30 checks (26 h stays the documented general-purpose default; a missed 03:15 run is caught that morning — tested with controlled timestamps); (2) checksum computed and written BEFORE publication, sidecar published first, published pair re-verified, checker enforces a sidecar for every backup from the first sidecar-bearing one on (the 9 pre-sidecar files stay valid); (3) publication by hard link — a late name collision is never clobbered (tested with and without a colliding sidecar); (4) failure paths remove only the run's own temporary files and half-published sidecar, never prune; 37 harness assertions green, 13 red against the C1 scripts | implemented, not deployed |
| Still open | schedule not installed; off-host copy unverified (the `objectCreator`-only + `rclone --immutable` design needs a permissions test); the live `dropdb`/`createdb` restore is a disaster-recovery plan never rehearsed on the running stack; deploy rollback unexercised | open |
...
| 9.6 | P9 Backups and restore | VPS infra (`docker/scripts/backup-postgres.sh`) | pg_dump inside the container, gzip, 7 retained; cron "documented only"; restore procedure documented in `docs/HOSTINGER_VPS_DEPLOYMENT.md` §7 | — | **G-6 (§1.6):** 7 manual dumps, nothing scheduled, no off-host copy, newest dump pre-B20 (F0); isolated restore of it passed. **G-6 C1 (§1.7):** fresh F2 dump `leadcapture-20260918-234035.sql.gz` restore-verified; hardened script + tests + proposals. **G-6 C2 (§1.8):** bridging F2 dump `leadcapture-20260922-100447.sql.gz` restore-verified with `KEEP=9`; missed-run detection (4 h post-run limit), checksum-before-publish, no-clobber publication and their tests on `claude/b23-g6c2-backup-guard` | **restore into a disposable container: verified (F0 and F2 ×2)**; **live restore: DR plan, not rehearsed**; **schedule: not activated**; **off-host: unverified** | deploy the C2 script, install the schedule with the 4 h post-run checks, test the uploader permissions, then a separately approved live-restore / rollback drill |
| 9.7 | P9 Rollback | `docker/scripts/deploy-vps.sh` | auto-rollback to `previous-deploy.sha` on failed health check; manual one-liner | — | pointer present (`ff7b851`; the commit is in the hosted checkout and on `origin/develop`); images carry only the `latest` tag (no SHA-tagged image, 0 dangling), so a rollback is a rebuild from source at `ff7b851` — the last two deploys took ≈3.5 min end-to-end; `ff7b851`→`f53c901` carried no schema change, so a code rollback is schema-compatible today; auto-rollback **never exercised** (17/17 deploys succeeded) | **implemented, not hosted-verified** (assessed read-only in G-6) | optional drill: deploy `ff7b851` then `f53c901` through the normal workflow while no schema change is pending |
...
| **G-6** | P2 (operations) | **partly closed** — isolated restores passed for the 2026-09-15 (F0), 2026-09-18 (F2) and 2026-09-22 (F2, bridging) dumps (§1.6–§1.8); the backup automation (hardened script, missed-run detection, checksum-before-publish, no-clobber) awaits review; **still open:** the daily schedule is not installed, no off-host copy is verified, the live `dropdb`/`createdb` restore is an unrehearsed disaster-recovery plan, deploy rollback is unexercised, root's crontab remains unreadable | a VPS loss loses everything since the last dump (now the 2026-09-22 bridging dump until the schedule is activated) | §1.6, §1.7, §1.8; runs 35404789095 / 35405027051 / 35406595433 / 35713935672 | (1) deploy the C2 script and install the two crontab lines with `BACKUP_MAX_AGE_HOURS=4` post-run checks (`docs/BACKUP_AND_RECOVERY.md` §3); (2) permissions test of the write-only uploader with `rclone copy --immutable`, then the off-host bucket + read-only verifier (§5); (3) land the scheduled freshness check (4 h) on the default branch; (4) separately approved drill of the live restore plan and of the deploy rollback | daily dump with log evidence and a 4 h check; off-host object listed by the read-only principal; drills return the stack to `f53c901` | owner approval for each operational change |
...
| Backups (cron, off-host) and restore rehearsal | **isolated restore verified (F0 + F2 ×2); schedule not activated; off-host unverified; live restore = unrehearsed DR plan** | G-6, §1.6–§1.8 |
| Rollback drill | **not yet checked** (mechanism present; assessed read-only in §1.6) | G-6 |
...
5. **G-6** — read-only crontab/backup listing in the same ops run; then, with approval, a restore rehearsal into a scratch database and an optional rollback drill. **Inventory + isolated restore rehearsal done 2026-09-18** (runs 35404789095 / 35405027051 on the temporary branch `ops/b23-g6-backup-rehearsal`, §1.6); remaining: schedule installation, off-host copy, script hardening, optional rollback drill — each an approved operational change. **Correction 1 done 2026-09-18** (run 35406595433, §1.7): fresh F2 backup captured with `KEEP=8` and restore-verified; hardened script, tests and proposals on `claude/b23-g6c1-backup-automation`; remaining = schedule activation, off-host copy, optional rollback drill. **Correction 2 done 2026-09-22** (run 35713935672, §1.8): bridging F2 backup with `KEEP=9` restore-verified; missed-run detection, checksum-before-publish and no-clobber publication with tests on `claude/b23-g6c2-backup-guard`; remaining = schedule activation, uploader permissions test + off-host verification, live-restore and rollback drills.
