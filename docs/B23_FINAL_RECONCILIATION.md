...
| 9.6 | P9 Backups and restore | VPS infra (`docker/scripts/backup-postgres.sh`) | pg_dump inside the container, gzip, 7 retained; cron "documented only"; restore procedure documented in `docs/HOSTINGER_VPS_DEPLOYMENT.md` §7 | — | **G-6 (§1.6):** 7 manual dumps, nothing scheduled, no off-host copy, newest dump pre-B20 (F0); isolated restore of it passed. **G-6 C1 (§1.7):** one fresh dump of the current schema taken with the deployed script (`leadcapture-20260918-234035.sql.gz`, F2, 72 tables, 3,525 rows) and restore-verified in a disposable container; hardened script + tests + schedule/off-host proposals on `claude/b23-g6c1-backup-automation` | **restore: verified (F0 and F2)**; **schedule: not activated**; **off-host: unknown / none** | install the proposed schedule; configure and independently verify the off-host copy (G-6) |
...
| **G-6** | P2 (operations) | **partly closed** — restore rehearsals passed for the 2026-09-15 (F0) and the fresh 2026-09-18 (F2) dumps (§1.6, §1.7); a hardened script, tests and the schedule/off-host proposals await review; **still open:** the daily schedule is not installed, no off-host copy is evidenced, rollback is unexercised, root's crontab remains unreadable | a VPS loss loses everything since the last dump (now ≤ the last manual run until the schedule is activated) | §1.6, §1.7; runs 35404789095 / 35405027051 / 35406595433 | (1) deploy the hardened script and install the two crontab lines (`docs/BACKUP_AND_RECOVERY.md` §3); (2) create the off-host bucket + write-only/read-only service accounts and verify from the destination (§5); (3) land the scheduled freshness check on the default branch; (4) optional rollback drill | daily dump with log evidence; off-host object listed by the read-only principal; drill returns to `f53c901` | owner approval for each operational change |
...
| Backups (cron, off-host) and restore rehearsal | **restore verified (F0 + fresh F2); schedule not activated; off-host unknown** | G-6, §1.6, §1.7 |
