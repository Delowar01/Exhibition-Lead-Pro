...
| **G-2** | **P1 (gate-blocking)** | the documented "restart the API, run once" procedure no longer yields a green API run: the suite's own intentional failed logins exceed `LOGIN_RATE_MAX=20`/15 min from one IP, producing 3 failures and 20 skips (§3.1) | the release gate cannot be evaluated from a single run; reruns would be needed, which the working rules forbid | §3.1 (9 × 429 on `/auth/login`); `config.ts:194`, `rateLimit.ts:24-31` | document and export `LOGIN_RATE_MAX` (e.g. 200) in the **local test shell only** (`docs/LOCALHOST_DEVELOPMENT.md` §3/§7, like the billing variables) — the production default stays 20; alternatively move the intentional-failure suites behind a per-suite IP header is not possible (single IP), so the env approach is the smallest | full API run: 1231 passed / 9 storage-gated failed / 28 skipped of 1268 (after G-1) | none |
...
2. **G-2** — document and export `LOGIN_RATE_MAX` for the local test shell in `docs/LOCALHOST_DEVELOPMENT.md` (docs-only commit; production default untouched); confirm one full API run shows only the storage-gated set (expected 1231 passed / 9 failed / 28 skipped of 1268 after step 1).
