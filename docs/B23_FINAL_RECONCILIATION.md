...
| 9.6 | P9 Backups and restore | VPS infra (`docker/scripts/backup-postgres.sh`) | pg_dump inside the container, gzip, 7 retained; cron "documented only"; restore procedure documented in `docs/HOSTINGER_VPS_DEPLOYMENT.md` §7 | — | **G-6 (2026-09-18, runs 35404789095 / 35405027051, §1.6):** 7 local dumps (2026-09-02 … 2026-09-15; all gzip-valid, complete, `600` in a `700` directory outside the volume), **all taken manually by activation runs — no cron entry, systemd timer or workflow schedules the script and `backup.log` is absent**; no off-host copy evidenced (no tool config for the deploy user, 0 backup-like objects in the dev bucket, Hostinger snapshots not verifiable from the VPS); the newest dump was 79 h old and predates the B20 schema change (fingerprint F0, 68 tables, vs live F2, 72); **isolated restore rehearsed**: same-image, network-less disposable container, exit 0, 0 SQL errors, 68 tables / 141 FKs / 3,206 rows identical to the dump's COPY blocks, restore 0.87 s + verify 0.59 s, cleanup verified, live stack unchanged | **restore: verified (rehearsal)**; **schedule: failed (nothing scheduled)**; **off-host: unknown / none found** | install and evidence the daily job, add an off-host copy, take a fresh dump after every schema change (G-6) |
...
| **G-6** | P2 (operations) | **partly closed 2026-09-18** — the isolated restore rehearsal passed (§1.6); **still open:** nothing schedules the backup (7 manual dumps only; the newest was 79 h old and pre-B20-schema), no off-host copy is evidenced, rollback is unexercised | a VPS loss loses everything since the last manual dump, and the newest dump cannot reproduce the current schema state | §1.6; runs 35404789095 / 35405027051 | (1) install the documented cron line for the deploy user and keep `backup.log` (evidence: `crontab -l`, log lines, a new dump within 24 h); (2) copy `/opt/lead-capture-pro/backups/postgres/` off-host and evidence it from the destination; (3) take a fresh dump after every schema change and before any rollback; (4) harden the script (write to `.tmp` then rename, `gzip -t`, checksum file, `flock`, failure alerting); (5) optional rollback drill | daily dump visible with log evidence; off-host object listed from the destination; drill returns to `f53c901` | owner approval for each operational change |
...
| Backups (cron, off-host) and restore rehearsal | **restore rehearsal passed; schedule failed (none); off-host unknown** | G-6, §1.6 |
