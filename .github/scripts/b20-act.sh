...
phase_cleanup() {
  ...
  local before; before="$(q "select 'companies='||(select count(*) from companies where id not in ($C))||' users='||(select count(*) from users where id not in ($U) and company_id is distinct from all(array[$C]))||' subscriptions='||(select count(*) from subscriptions where company_id not in ($C))||' audit='||(select count(*) from audit_logs where (company_id is null or company_id not in ($C)) and (user_id is null or user_id not in ($U)))||' activity='||(select count(*) from activity_logs where (company_id is null or company_id not in ($C)) and (user_id is null or user_id not in ($U)))")"; echo "$before"
