...
queue_checks() {
  ...
  Q_LINE="$(printf '%s\n' "$API_LOG" | grep 'Durable job queue started' | tail -1)"
...
do_apply() {
  ...
  # Every reader below consumes the whole stream (no early exit): with pipefail an
  # early-terminating reader would SIGPIPE gunzip and abort the script (exit 141).
  CO_COPY_ROWS="$(gunzip -c "$BACKUP_DIR/$BACKUP_FILE" | awk '/^COPY public\.companies /{f=1;next} f&&/^\\\.$/{f=0} f{n++} END{print n+0}')"
  N_CO_TABLE="$(gunzip -c "$BACKUP_DIR/$BACKUP_FILE" | grep -c '^CREATE TABLE public\.companies (' || true)"
  [ "$N_CO_TABLE" = "1" ] || fail "backup does not contain the companies table"
  N_COMPLETE="$(gunzip -c "$BACKUP_DIR/$BACKUP_FILE" | grep -c 'PostgreSQL database dump complete' || true)"
  [ "$N_COMPLETE" = "1" ] || fail "backup is not a complete pg_dump"
  ...
  N_IDX_B="$(printf '%s' "$SCHEMA_COUNTS" | grep -oE '[0-9]+ indexes' || true)"; N_IDX_A="$(printf '%s' "$SCHEMA_COUNTS_AFTER" | grep -oE '[0-9]+ indexes' || true)"
  N_CON_B="$(printf '%s' "$SCHEMA_COUNTS" | grep -oE '[0-9]+ constraints' || true)"; N_CON_A="$(printf '%s' "$SCHEMA_COUNTS_AFTER" | grep -oE '[0-9]+ constraints' || true)"
  N_TAB_B="$(printf '%s' "$SCHEMA_COUNTS" | grep -oE '^[0-9]+ tables' || true)"; N_TAB_A="$(printf '%s' "$SCHEMA_COUNTS_AFTER" | grep -oE '^[0-9]+ tables' || true)"
  [ -n "$N_IDX_B" ] && [ -n "$N_CON_B" ] && [ -n "$N_TAB_B" ] || fail "could not parse the schema counts"
