...
preflight() {
  ...
  printf '%s\n' "$API_LOG" | grep -q 'Durable job queue selected' || fail "durable queue NOT selected in the running api"
  printf '%s\n' "$API_LOG" | grep -q 'Durable job queue started'  || fail "durable queue NOT started in the running api"
  printf '%s\n' "$API_LOG" | grep -qE 'Background job queue started|Unknown JOBS_DRIVER|requires JOBS_PAYLOAD_ENCRYPTION_KEY' && fail "api is NOT on the durable driver"
...
do_apply() {
  ...
  printf '%s\n' "$TDIFF" | grep -E '^<' | grep -q . && fail "a table disappeared or lost columns"
  printf '%s\n' "$TDIFF" | grep -E '^>' | grep -vqE '^> workflow_runs\(19\)$|^> workflow_action_runs\(13\)$' && fail "unrelated table change detected"
  ...
  docker port "$WEB_CID" | grep -q '127.0.0.1:18080' || fail "web is no longer bound to 127.0.0.1:18080"
...
do_smoke() {
  ...
  printf '%s\n' "$API_LOG" | grep -q 'Durable job queue selected' || fail "durable queue NOT selected"
  printf '%s\n' "$API_LOG" | grep -q 'Durable job queue started'  || fail "durable queue NOT started"
  printf '%s\n' "$API_LOG" | grep -qE 'Background job queue started|Unknown JOBS_DRIVER|requires JOBS_PAYLOAD_ENCRYPTION_KEY|"driver":"in-process"' && fail "in-process fallback detected"
  printf '%s\n' "$API_LOG" | grep -q '"driver":"postgres"' || fail "durable queue driver line does not say postgres"
  printf '%s\n' "$API_LOG" | grep -q 'Recurring task scheduler started' || fail "scheduler not started"
  ...
  printf '%s' "$R_A_RES" | grep -q "\"tagId\":$TAG_ID" || fail "action result does not reference tag $TAG_ID"
  ...
  docker port "$WEB_CID" | grep -q '127.0.0.1:18080' || fail "web binding changed"
