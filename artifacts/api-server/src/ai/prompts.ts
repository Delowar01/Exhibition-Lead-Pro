...
- "address": physical street address (display value), or null
- "city": city/town printed on the card (display value), or null
- "country": country printed on the card (display value), or null — only when it is actually printed or unambiguously implied by the printed address; do NOT infer from phone codes
- "postalCode": postal/ZIP code printed on the card, or null
- "original": an object holding the SAME keys (firstName, lastName, arabicName, jobTitle, company, email, mobile, website, linkedin, address, city, country, postalCode) with the text EXACTLY as printed on the card — NO translation, NO transliteration, verbatim original script. Use null for any field not present.
