...
//      external-account tokens + sidecar-brokered signed URLs. Requires no
...
//      gcloud ADC, or GCE/GKE metadata. Signed URLs are minted locally with the
//      client library (requires a service account with signBlob capability).
...
async function signObjectURL({
  ...
  // Portable path: V4 signed URL minted by the client library using standard
  // Google credentials (no sidecar involved).
  if (!usingReplitSidecar) {
    const action = (
      { GET: "read", PUT: "write", DELETE: "delete", HEAD: "read" } as const
    )[method];
